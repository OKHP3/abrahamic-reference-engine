...
| Confirm `.agents/memory/threads/` as the tracked repository location. | user + agent | complete | Repository governance review | The extracts are stored under the permitted `.agents/` tree; the legacy root-level `context/` directory was removed after relocation. |
...
10. Should `.agents/memory/threads/` receive a formal inventory entry in `AGENTS.md`? The location is governed by the existing `.agents/` allowlist, and the current inventory update is pending.
...
- **Repository boundary note:** The original request named `context/threads/`, but the artifact was relocated to `.agents/memory/threads/` because the root-level `context/` directory is outside the repository allowlist.
