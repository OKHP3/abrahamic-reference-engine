...
|---|---|---|---|---|
| Preserve this extract under `.agents/memory/threads/`. | agent | complete | Reviewed body and extractor utility | Final Markdown exists in the governed memory location and passes utility validation. |
...
- **Repository governance note:** The original request named `context/threads/`, but the repository's AGENTS.md permitted-root list does not include that root-level directory. This extract now lives under `.agents/memory/threads/`, which is inside the permitted `.agents/` tree.
