...
test.describe('keyboard, focus, zoom, and motion accessibility', () => {
  ...
  test('preserves mobile navigation state across browser history', async ({ page }) => {
    ...
    const expectClosedNavigation = async (path: string, heading: string) => {
    ...
    await expectClosedNavigation('/browse/catholic', 'Catholic')

    await openButton.click()
    await expect(sidebar).toBeVisible()
    const islamLink = sidebar.getByRole('link', { name: /^Islam/ }).first()
    await islamLink.click()
    ...
    await page.goBack()
    await expectClosedNavigation('/browse', 'Browse Traditions')
  ...
  test('keeps keyboard-selected traditions closed through browser history', async ({ page }) => {
    test.skip((page.viewportSize()?.width ?? 0) >= 768, 'Mobile navigation is hidden on desktop')
    await page.goto(`${pagesBasePath}/browse`)
    ...
    await selectTradition(/Catholic/, '/browse/catholic', 'Catholic')
    await selectTradition(/^Islam/, '/browse/islam', 'Islam')
